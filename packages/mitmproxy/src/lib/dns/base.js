...
module.exports = class BaseDNS {
  ...
  async lookup (hostname) {
    try {
      ...
      log.info(`[DNS '${this.dnsName} - ${this.dnsType}'] ${hostname} ➜ ${ipCache.value} (${new Date() - t} ms), ipList: ${JSON.stringify(ipList)}, ipCache:`, JSON.stringify(ipCache))

      return ipCache.value
    } catch (error) {
      log.error(`[DNS '${this.dnsName} - ${this.dnsType}'] cannot resolve hostname ${hostname}, error:`, error)
      return hostname
    }
  }
  ...
  async _lookup (hostname) {
    ...
    try {
      ...
      if (response == null || response.answers == null || response.answers.length == null || response.answers.length === 0) {
        // 说明没有获取到ip
        log.warn(`[DNS '${this.dnsName} - ${this.dnsType}'] 没有该域名的IP地址: ${hostname}, cost: ${cost} ms, response:`, response)
        return []
      }
      const ret = response.answers.filter(item => item.type === 'A').map(item => item.data)
      if (ret.length === 0) {
        log.info(`[DNS '${this.dnsName} - ${this.dnsType}'] 没有该域名的IPv4地址: ${hostname}, cost: ${cost} ms`)
      } else {
        log.info(`[DNS '${this.dnsName} - ${this.dnsType}'] 获取到该域名的IPv4地址： ${hostname} - ${JSON.stringify(ret)}, cost: ${cost} ms`)
      }
      return ret
    } catch (e) {
      log.error(`[DNS '${this.dnsName} - ${this.dnsType}'] DNS query error, hostname: ${hostname}${this.dnsServer ? `, dnsServer: ${this.dnsServer}` : ''}, cost: ${Date.now() - start} ms, error:`, e)
      return []
    }
  }
}
