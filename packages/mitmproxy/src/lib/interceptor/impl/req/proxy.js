...
module.exports = {
  ...
  requestIntercept (context, interceptOpt, req, res, ssl, next, matched) {
    ...
    if (interceptOpt.sni) {
      ...
      rOptions.servername = interceptOpt.sni === '[empty]' ? '' : interceptOpt.sni
      if (interceptOpt.ssl !== true && rOptions.agent.options.rejectUnauthorized && rOptions.agent.unVerifySslAgent) {
